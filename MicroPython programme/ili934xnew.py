    ...
    def printEmanVerzija(self, text):
        ...
        for line in lines:
            ...
            for i, word in enumerate(words):
                word_width = self._font.get_width(word)

                # Check if the word fits within the remaining line width
                if curx + word_width < self.width:
                    ...
                    while word_width > self.width:
                        # Print a portion of the word that fits on a single line
                        partial_word = word[:self.width // char_w]
                        curx = self.chars(partial_word, curx, cury)

                        # Remove the printed portion from the word
                        word = word[self.width // char_w:]

                        # Start a new line for the remaining portion of the word
                        curx = self._x
                        cury = self.next_line(cury, char_h)

                    # Print the remaining portion of the word with a space after it
                    curx = self.chars(word + ' ', curx, cury)
